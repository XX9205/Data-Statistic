...
def data_process():
    ...
    time_interval = [2, 4, 9, 5, 2, 8, 2, 7, 7, 1, 8, 6, 12, 3, 2, 3, 4, 2, 2, 2]    #fattree topo
    ...
    jitter_ave = {}
    loss_ave = {}
    traffic_load = [40, 50, 60, 70, 80, 90]             #   10, 20, 30, 40, 50, 60, 70, 80, 90
    for load in traffic_load:
        ...
        for file_num in range(1, 11):
            ...
            for server_num in range(0, 16):
                file_object = open(r"C:\Users\dell\Desktop\Fattree测试数据\ECMP" + "\\" + str(load) + "\\" + str(file_num) + r"\server" + str(server_num) + '.txt')
            ...
            avebw_list.append(float('%.5f' % (avebw_sum / 16)))
            avejitter_list.append(float('%.5f' % (jitter_sum / 16)))
            aveloss_list.append(float('%.5f' % (loss_sum / 16)))

            bw_sum = copy.deepcopy(bw_data[0])
            for text_num in range(1, 16):
    ...
    data_text = open(r'C:\Users\dell\Desktop\Fattree测试数据\ECMP\data.txt', 'w')
    print(data, file=data_text)
    data_text.close()

    bw_ave_text = open(r'C:\Users\dell\Desktop\Fattree测试数据\ECMP\bw_ave.txt', 'w')
    print(bw_ave, file=bw_ave_text)
    bw_ave_text.close()

    jitter_ave_text = open(r'C:\Users\dell\Desktop\Fattree测试数据\ECMP\jitter_ave.txt', 'w')
    print(jitter_ave, file=jitter_ave_text)
    jitter_ave_text.close()

    loss_ave_text = open(r'C:\Users\dell\Desktop\Fattree测试数据\ECMP\loss_ave.txt', 'w')
    print(loss_ave, file=loss_ave_text)
    loss_ave_text.close()

    data_traffic_text = open(r'C:\Users\dell\Desktop\Fattree测试数据\ECMP\data_traffic.txt', 'w')
