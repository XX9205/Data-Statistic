...
def data_process():
    ...
    jitter_ave = {}
    loss_ave = {}
    traffic_load = [10, 20]             #   10, 20, 30, 40, 50, 60, 70, 80, 90
    for load in traffic_load:
        ...
        for file_num in range(1, 4):
            ...
            for server_num in range(0, 16):
                file_object = open(r"C:\Users\dell\Desktop\DTS-Fattree测试数据\threshold\0.03" + "\\" + str(load) + "\\" + str(file_num) + r"\server" + str(server_num) + '.txt')
    ...
    data_text = open(r'C:\Users\dell\Desktop\DTS-Fattree测试数据\data.txt', 'w')
    print(data, file=data_text)
    data_text.close()

    bw_ave_text = open(r'C:\Users\dell\Desktop\DTS-Fattree测试数据\bw_ave.txt', 'w')
    print(bw_ave, file=bw_ave_text)
    bw_ave_text.close()

    jitter_ave_text = open(r'C:\Users\dell\Desktop\DTS-Fattree测试数据\jitter_ave.txt', 'w')
    print(jitter_ave, file=jitter_ave_text)
    jitter_ave_text.close()

    loss_ave_text = open(r'C:\Users\dell\Desktop\DTS-Fattree测试数据\loss_ave.txt', 'w')
    print(loss_ave, file=loss_ave_text)
    loss_ave_text.close()

    data_traffic_text = open(r'C:\Users\dell\Desktop\DTS-Fattree测试数据\data_traffic.txt', 'w')
